...
import wandb
from models.dynamic.all_depth import MatVisionTransformer, ModifiedBlock
    ...
    for epoch in range(args.epochs):
        with tqdm(total=len(trainDataLoader), postfix=dict, mininterval=0.3) as pbar:
            ...
            for batch_idx, (img, label) in enumerate(trainDataLoader):
                ...
                preds, attn_mask, mlp_mask, embed_mask = model(img)
                # preds, cost = model(img)

                ce_loss = criterion(preds, label)

                latency_loss = torch.square(latency-attn_mask) + torch.square(latency-mlp_mask) + torch.square(latency-embed_mask)
