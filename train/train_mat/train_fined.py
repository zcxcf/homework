...
from models.all_elastic.fined_elastic import MatVisionTransformer
...
from utils.eval_flag import eval_mat, eval_mat_fined
import random
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(trainDataLoader), postfix=dict, mininterval=0.3) as pbar:
            ...
            if epoch in args.stage_fined_epochs:
                stage_index = args.stage_fined_epochs.index(epoch)
            ...
            for batch_idx, (img, label) in enumerate(trainDataLoader):
                ...
                model.configure_subnetwork_fined(num_head=r+3)
                preds = model(img)
        ...
        if epoch % 2 == 0:
            for f in flags_list:
                eval_mat_fined(model, valDataLoader, criterion, epoch, optimizer, args, flag=f)
