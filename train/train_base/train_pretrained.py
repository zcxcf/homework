...
def train(args):
    ...
    model_path = '/home/ssd7T/zc_reuse/iccv/pretrained_para/vit_base.pth'
    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(trainDataLoader), postfix=dict, mininterval=0.3) as pbar:
            ...
            for batch_idx, (img, label) in enumerate(trainDataLoader):
                ...
                optimizer.zero_grad()
                model.configure_subnetwork(flag='l')
                preds = model(img)
        ...
        if epoch % 2 == 0:
            ...
            with tqdm(total=len(valDataLoader), postfix=dict, mininterval=0.3) as pbar:
                ...
                with torch.no_grad():
                    ...
                    wandb.log({"Epoch": epoch + 1, "Val Loss_l": avg_loss})
                    wandb.log({"Epoch": epoch + 1, "Val Acc_l": accuracy})
