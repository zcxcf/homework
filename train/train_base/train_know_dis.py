    ...
    for epoch in range(args.epochs):
        ...
        with tqdm(total=len(trainDataLoader), postfix=dict, mininterval=0.3) as pbar:
            ...
            for batch_idx, (img, label) in enumerate(trainDataLoader):
                ...
                with torch.no_grad():
                    model.configure_subnetwork(flag='s')
                    tiny_logist = model(img)

                hard_loss = criterion(preds, label)
                soft_loss = distillation_loss(student_logits=preds, teacher_logits=tiny_logist)
                loss = hard_loss+soft_loss

                if batch_idx % 10 == 0:
                    wandb.log({"train Batch Loss": hard_loss.item()})
                total_loss += loss.item()
                total_dis_loss += soft_loss.item()
            ...
            epoch_dis_loss = total_dis_loss / len(trainDataLoader)
            print("train loss", epoch_loss)

            wandb.log({"Epoch": epoch + 1, "Train epoch Loss": epoch_loss})
            wandb.log({"Epoch": epoch + 1, "distillation Loss": epoch_dis_loss})
