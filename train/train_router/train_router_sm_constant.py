...
import random
from models.router.complex_sm import SM
from models.router.complex_router import Router
...
def train(args):
    ...
    sm = SM()
    sm.to(args.device)
    # check_point_path = '/home/ssd7T/zc_reuse/iccv/router_logs_weight/sm_constantnone_cifar100/Feb12_13-02-18/weight/sm.pth'
    # checkpoint = torch.load(check_point_path, map_location=args.device)
    ...
    optimizer_sm = torch.optim.SGD(sm.parameters(), lr=args.lr, weight_decay=0)
    ...
    for epoch in range(args.epochs):
        ...
        if epoch > 10:
            ...
            for i in range(accmulated_step):
                ...
                sub_dim, depth_list, mlp_ratio_list, mha_head_list, probs = router(x)

                sub_dim, depth_list, mlp_ratio_list, mha_head_list, preds = sm(probs)

                pred_model_loss = preds[0][0]
                pred_macs = preds[0][1]

                if x<pred_macs:
                    router_loss = pred_model_loss + torch.square((pred_macs - x))
                else:
                    router_loss = pred_model_loss
        ...
        if epoch < 11 or epoch > 20:
            epoch_sm += 1

            adjust_learning_rate(optimizer_sm, epoch_sm + 1, args)
            router.eval()
            sm.train()
            ...
            for i in range(accmulated_step):
                ...
                if epoch < 11 or torch.rand((1)) > 0.5:
                    probs = torch.rand((1, 37)).to(args.device)
                # probs += (torch.rand((1, 37)).to(args.device)-0.2)
        ...
        with torch.no_grad():
            ...
            for index, f in enumerate(flags_list):
                x = torch.tensor([[index*0.4]]).to(args.device)*5
