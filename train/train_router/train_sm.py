...
def train(args):
    ...
    optimizer = torch.optim.AdamW(sm.parameters(), lr=args.lr)
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(accmulated_step):
            ...
            x = torch.rand((1, 37)).to(args.device)

            sub_dim, depth_list, mlp_ratio_list, mha_head_list, preds = sm(x)
            pred_loss = preds[0][0]
            pred_macs = preds[0][1]
            ...
            with torch.no_grad():
                for batch_idx, (img, label) in enumerate(valDataLoader):
                    ...
                    break
            loss = F.mse_loss(input=pred_loss, target=train_loss)+F.mse_loss(input=pred_macs, target=macs)
            wandb.log({"model_train_loss": train_loss.item()})
            wandb.log({"pred_train_loss": pred_loss.item()})
            wandb.log({"macs": macs})
            wandb.log({"pred_macs": pred_macs.item()})
