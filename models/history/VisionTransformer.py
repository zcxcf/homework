...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            global_pool: Type of global pooling for train_final sequence (default: 'token').
...
def init_weights_vit_moco(module: nn.Module, name: str = '') -> None:
    ...
    if isinstance(module, nn.Linear):
        if 'qkv' in name:
            # treat the trained_weight of Q, K, V separately
...
def _load_weights(model: VisionTransformer, checkpoint_path: str, prefix: str = '') -> None:
    """ Load trained_weight from .npz checkpoints for official Google Brain Flax implementation
    """
    import numpy as np
    ...
    if pos_embed_w.shape != model.pos_embed.shape:
        ...
        pos_embed_w = resample_abs_pos_embed(  # resize pos embedding when different size from pretrained trained_weight
            pos_embed_w,
    ...
    # NOTE representation layer has been removed, not used in latest 21k/1k pretrained trained_weight
    ...
    for k, v in state_dict.items():
        ...
        if k == 'proj':
            ...
            if v.shape[1] != model.pos_embed.shape[1]:
                # To resize pos embedding when using model at different size from pretrained trained_weight
                v = resize_pos_embed(
                    v,
                    model.pos_embed,
...
def checkpoint_filter_fn(
    ...
    if 'visual.trunk.pos_embed' in state_dict:
        ...
        # FIXME remap train_final nn.Linear if it exists outside of the timm .trunk (ie in visual.head.proj)
        prefix = 'visual.trunk.'
    ...
    for k, v in state_dict.items():
        if 'patch_embed.proj.weight' in k:
            ...
            # NOTE representation layer removed as not used in latest 21k/1k pretrained trained_weight
            continue
        out_dict[k] = v
    return out_dict
