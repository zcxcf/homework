...
class MatVisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def caculate_cost(self):
        ...
        for i in range(self.depth):
            ...
            attn_macs = (3 * N * d_embed * d_head * num_head + 2 * N * N * d_head * num_head) * self.depth_attn_mask[i]

            ratio = torch.sum(self.blocks[i].mlp.mask) / 2

            mlp_macs = 2 * N * d_embed * self.embed_dim * ratio * self.depth_mlp_mask[i]
    ...
    def configure_latency(self, latency):
        ...
        self.hard = True
        for layer_idx in range(self.depth):
            self.blocks[layer_idx].attn.configure_latency(latency, tau=1, hard=True)
            self.blocks[layer_idx].mlp.configure_latency(latency, tau=1, hard=True)
...
if __name__ == '__main__':
    ...
    embed_mask = torch.tensor([1.0]*11).to(device)
