...
class ModifiedVitMlp(nn.Module):
    ...
    def __init__(
        ...
        min_dim = self.hidden_features//8
        self.mask = torch.cat((torch.ones(min_dim), torch.zeros(self.hidden_features-min_dim)), dim=0).to(self.fc1.weight.device)
    ...
    def forward(self, x):
        ...
        x_middle = x_middle*self.mask[:self.current_subset_hd]
    ...
    def updata_mask(self, step):
        self.mask[:self.current_subset_hd] = self.mask[:self.current_subset_hd]+step
...
if __name__ == '__main__':
    model = MatVisionTransformer(
        patch_size=16, embed_dim=192, depth=12, num_heads=3, mlp_ratio=32, qkv_bias=True,
    ...
    print(model.blocks[0].mlp.current_subset_dim)
    ...
    print(out.shape)
    print()
    print(model.blocks[0].mlp.mask.requires_grad)
