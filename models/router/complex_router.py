...
class Router(nn.Module):
    def __init__(self):
        super().__init__()
        self.linear1 = nn.Linear(in_features=1, out_features=(1 + 12 + 12 + 12)*2)
        self.act = nn.LeakyReLU()
        self.linear2 = nn.Linear(in_features=(1 + 12 + 12 + 12)*2,
                                 out_features=(1 + 12 + 12 + 12)*2)
        self.act2 = nn.LeakyReLU()
        self.linear3 = nn.Linear(in_features=(1 + 12 + 12 + 12) * 2,
                                 out_features=(1 + 12 + 12 + 12))
        self.softmax = nn.Softmax()
        self.sigmoid = nn.Sigmoid()

    def forward(self, latency):
        probs = self.sigmoid(self.linear3(self.act2(self.linear2(self.act(self.linear1(latency))))))
        # print(probs)
