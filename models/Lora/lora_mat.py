...
class ModifiedAttention(nn.Module):
    ...
    def __init__(
        ...
        rank = 8
        in_features, out_features = self.qkv.weight.size()
        self.qkv_A = nn.Parameter(torch.randn(in_features, rank))
        self.qkv_B = nn.Parameter(torch.zeros(rank, out_features))

        in_features, out_features = self.proj.weight.size()
        self.proj_A = nn.Parameter(torch.randn(in_features, rank))
        self.proj_B = nn.Parameter(torch.zeros(rank, out_features))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        lora_qkv_weight = self.qkv.weight+torch.matmul(self.qkv_A, self.qkv_B)

        sub_qkv_weight = torch.cat((lora_qkv_weight[:self.current_subset_dim, :self.current_subset_dim],
                                         lora_qkv_weight[r*self.current_subset_dim:(r+1)*self.current_subset_dim, :self.current_subset_dim],
                                         lora_qkv_weight[r*2*self.current_subset_dim:(r*2+1)*self.current_subset_dim, :self.current_subset_dim]), dim=0)

        sub_qkv_bias = torch.cat((self.qkv.bias[:self.current_subset_dim],
        ...
        qkv_out = F.linear(sub_input, sub_qkv_weight, bias=sub_qkv_bias)
        ...
        sub_proj_weight = (self.proj.weight+torch.matmul(self.proj_A, self.proj_B))[:self.current_subset_dim, :self.current_subset_dim]
        # print("weight", self.sub_proj_weight[0, :10])
        sub_proj_bias = self.proj.bias[:self.current_subset_dim]
        # print("bias", self.sub_proj_bias[:10])
        proj_output = F.linear(qkv_output, sub_proj_weight, bias=sub_proj_bias)
...
class ModifiedVitMlp(nn.Module):
    def __init__(
        ...
        in_features, out_features = self.fc1.weight.size()
        self.fc1_A = nn.Parameter(torch.randn(in_features, rank))
        self.fc1_B = nn.Parameter(torch.zeros(rank, out_features))

        in_features, out_features = self.fc2.weight.size()
        self.fc2_A = nn.Parameter(torch.randn(in_features, rank))
        self.fc2_B = nn.Parameter(torch.zeros(rank, out_features))
    ...
    def forward(self, x):
        ...
        up_proj = (self.fc1.weight+torch.matmul(self.fc1_A, self.fc1_B))[:self.current_subset_dim*4, :self.current_subset_dim]
        up_bias = self.fc1.bias[:self.current_subset_dim*4]

        down_proj = (self.fc2.weight+torch.matmul(self.fc2_A, self.fc2_B))[:self.current_subset_dim, :self.current_subset_dim*4]
        down_bias = self.fc2.bias[:self.current_subset_dim]

        x_middle = F.linear(sub_input, up_proj, bias=up_bias)

        output = F.linear(self.act(x_middle), down_proj, bias=down_bias)
        # output = torch.cat((output, x[:, :, self.current_subset_dim:]), dim=2)
...
if __name__ == '__main__':
    ...
    for name, param in model.named_parameters():
        ...
        if "A" in name or 'B' in name or 'head' in name or "norm" in name:
