...
class ModifiedAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.training:
            ...
            mask = mask.unsqueeze(0).unsqueeze(0).expand(B, N, self.dim)
            ...
            # print(mask_triple.shape)
            # print(self.dim_bool.shape)

            # dim_bool = self.dim_bool.unsqueeze(0).expand(mask_triple.size(0), -1)  # 形状变为 [960, 256]
...
class ModifiedVitMlp(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            ...
            mask = mask.unsqueeze(0).unsqueeze(0).expand(b, n, 4 * self.embed_dim)
...
class MatVisionTransformer(timm.models.vision_transformer.VisionTransformer):
    ...
    def forward_features(self, x: torch.Tensor):
        ...
        if self.training:
            ...
            mask = mask.unsqueeze(0).unsqueeze(0).expand(B, N, self.embed_dim)
            x = x * mask
